...
def build_parser() -> argparse.ArgumentParser:
    ...
    sub.add_parser("ws-ping", help="Send a ping event to Kraken v2 and await pong.")
...
def cmd_check(settings: Settings) -> int:
    ...
    from .data.ws_client import _BookState, _compute_checksum_v2, _levels_from_v2

    logger = get_logger("check", settings.log_level)
    # 1) Clock quick tick
    clock = Clock(period_ms=100, logger=logger)
    ticks = {"n": 0}
    def cb(_i: int, _dt: float) -> None: ticks["n"] += 1
...
async def _ws_ping_async(settings: Settings) -> int:
    ...
    async def one_pong() -> int:
        # open a stream; _resilient_stream sends subscribe; we send a ping via private method
        async for _ in client._resilient_stream(channel="trade", symbols=["BTC/USD"]):
            return 0
        return 1

    return await one_pong()
